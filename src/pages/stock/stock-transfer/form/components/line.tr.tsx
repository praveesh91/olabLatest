...
import { castToNumber, displayNumber } from "../../../../../services/conversion.service";
...
const Line = ({
    ...
    const addQuantity = (addValue: number, originalValue: number) => {
        return originalValue + addValue;
    }

    const substractQuantity = (subValue: number, originalValue: number) => {
        return originalValue - subValue;
    }
    ...
    return (
        <tr className="align-middle inventory-form-tr">
            ...
            <td className="text-end">
                {disabled ? line["stockByWarehouse"]?.[fromWarehouse?.id]?.["in_stock"] : <div className="comparator-box right">
                    <div className="first-obj">
                        <div className="value">
                            {line["stockByWarehouse"]?.[fromWarehouse?.id]?.["in_stock"]}
                        </div>
                    </div>
                    <div className="symbol" hidden={line.quantity === "" ? true : false}>
                        <i className="fa fa-angle-right"></i>
                    </div>
                    <div
                        className="second-obj"
                        hidden={line.quantity === "" ? true : false}
                    >
                        <div className="value">
                            {/* {getFutureStockValue(
                                displayNumber(
                            ...
                            {
                                substractQuantity(castToNumber(line.quantity) || 0, castToNumber(line["stockByWarehouse"]?.[fromWarehouse?.id]?.["in_stock"]) || 0)
                            }
                        </div>
                    </div>
                </div>}
            ...
            <td className="text-end">
                {disabled ? line["stockByWarehouse"]?.[toWarehouse?.id]?.["in_stock"] : <div className="comparator-box right">
                    <div className="first-obj">
                        <div className="value">
                            {line["stockByWarehouse"]?.[toWarehouse?.id]?.["in_stock"]}
                        </div>
                    </div>
                    <div className="symbol" hidden={line.quantity === "" ? true : false}>
                        <i className="fa fa-angle-right"></i>
                    ...
                    >
                        <div className="value">
                            {
                                addQuantity(castToNumber(line.quantity) || 0, castToNumber(line["stockByWarehouse"]?.[toWarehouse?.id]?.["in_stock"]) || 0)
                            }
                        </div>
                    </div>
                </div>}
